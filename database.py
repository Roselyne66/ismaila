...
def archiver_mesure(model, r_batterie, soh_resistance, grade, valide=0):
    """
    ...
    with _connexion() as conn:
        conn.execute(
            "INSERT INTO mesures (date, model, r_batterie, soh_resistance, grade, valide) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (datetime.now().isoformat(timespec="seconds"),
             model, r_batterie, soh_resistance, grade, valide),
        )
    return True
