...
@app.get("/historique/{imei}")
def historique(imei: str):
    """
    Renvoie tous les contrôles d'un téléphone (par IMEI).
    C'est l'appel qui alimente le Passeport Santé : la vie complète du téléphone,
    contrôle après contrôle. Relie certificat, rapport et passeport par l'IMEI.
    """
    controles = database.obtenir_historique(imei)
    return {
        "imei": imei,
