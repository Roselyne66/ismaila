...
def lancer_diagnostic(data: dict) -> ResultatDiagnostic:
    ...
    # Référence "batterie neuve", par ordre de priorité :
    #   1. terrain (P5 base / table admin CEO)  → la plus fiable une fois la base remplie
    #   2. table constructeur embarquée          → au démarrage
    #   3. fallback universel 90 mΩ
    r_ref = database.obtenir_r_ref_terrain(model)
    if r_ref is None:
    ...
    # Seuil P97 : dynamique si la base est assez remplie, sinon fallback 350 mΩ.
    seuil_p97, source_p97 = database.obtenir_p97()
    if r_corrigee > seuil_p97:
        r._refuser(
            f"Résistance interne trop élevée : {r_corrigee} mΩ "
            f"(seuil {source_p97} : {seuil_p97} mΩ) — batterie très dégradée"
        )
